...
Build steps on Windows:
    ...
    python build.py configure -k 5.1 --moz-src=cvs --release --no-strip --tools --moz-objdir=ko-obj
...
Build steps on Linux and Mac OS X:
    ...
    #---- Build Mozilla
    cd openkomodo/mozilla
    python build.py configure -k 5.1 --moz-src=cvs --release \
        --no-strip --tools
    python build.py distclean all
    cd ..
    #---- Build Komodo
...
3. Configure for the mozilla build. On Windows you currently want
   ...
   - configure for a Komodo Edit 5.1.x build
...
2. Configure you Komodo build.
   ...
   "python configure.py -k 5.1 ..." then you'd want something like:
   
        bk configure -V 5.1.0-devel
