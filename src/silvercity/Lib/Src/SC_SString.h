...
// An SC_SString may contain embedded nul characters.

/**
 * Base class from which the two other classes (SBuffer & SC_SString)
 * are derived.
 */
class SContainer {
public:
 ...
 * This class is intended to be shortlived, to be transformed as SC_SString
 ...
 * we should set sLen to strlen(sb.ptr()) in related SC_SString constructor and assignment.
 */
 ...
 */
class SC_SString : protected SContainer {
	...
	SC_SString &assign(const char *sOther, lenpos_t sSize_=measure_length);

public:
	SC_SString() : sLen(0), sizeGrowth(sizeGrowthDefault) {}
	SC_SString(const SC_SString &source) : SContainer(), sizeGrowth(sizeGrowthDefault) {
		s = StringAllocate(source.s, source.sLen);
		sSize = sLen = (s) ? source.sLen : 0;
	}
	SC_SString(const char *s_) : sizeGrowth(sizeGrowthDefault) {
		s = StringAllocate(s_);
		sSize = sLen = (s) ? strlen(s) : 0;
	}
	SC_SString(SBuffer &buf) : sizeGrowth(sizeGrowthDefault) {
		s = buf.ptr();
		sSize = sLen = buf.size();
		// Consumes the given buffer!
		buf.reset();
	}
	SC_SString(const char *s_, lenpos_t first, lenpos_t last) : sizeGrowth(sizeGrowthDefault) {
	...
	SC_SString(int i);
	SC_SString(double d, int precision);
	~SC_SString() {
		sLen = 0;
	}
	void clear() {
	...
	SC_SString &operator=(const char *source) {
		return assign(source);
	}
	SC_SString &operator=(const SC_SString &source) {
		if (this != &source) {
			assign(source.s, source.sLen);
		}
		return *this;
	}
	bool operator==(const SC_SString &sOther) const;
	bool operator!=(const SC_SString &sOther) const {
	...
	SC_SString substr(lenpos_t subPos, lenpos_t subLen=measure_length) const;
	SC_SString &lowercase(lenpos_t subPos = 0, lenpos_t subLen=measure_length);
	SC_SString &uppercase(lenpos_t subPos = 0, lenpos_t subLen=measure_length);
	SC_SString &append(const char *sOther, lenpos_t sLenOther=measure_length, char sep = '\0');
	SC_SString &operator+=(const char *sOther) {
		return append(sOther, static_cast<lenpos_t>(measure_length));
	}
	SC_SString &operator+=(const SC_SString &sOther) {
		return append(sOther.s, sOther.sLen);
	}
	SC_SString &operator+=(char ch) {
		return append(&ch, 1);
	}
	SC_SString &appendwithseparator(const char *sOther, char sep) {
		return append(sOther, strlen(sOther), sep);
	}
	SC_SString &insert(lenpos_t pos, const char *sOther, lenpos_t sLenOther=measure_length);
	...
	SC_SString &change(lenpos_t pos, char ch) {
