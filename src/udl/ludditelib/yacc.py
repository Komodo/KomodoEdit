...
class YaccProduction:
    ...
    def pushback(self,n):
        ...
        if n > (len(self.slice)-1):
            raise_(ValueError, "Can't push %d tokens. Only %d are available." % (n,len(self.slice)-1))
        for i in range(0,n):
    ...
    if (optimize and lr_read_tables(tabmodule,1)):
        ...
        if ef:
            ...
            if (ef.__code__.co_argcount != 1+ismethod):
                raise_(YaccError,"%s:%d: p_error() requires 1 argument." % (efile,eline))
            global Errorfunc
            Errorfunc = ef
        else:
        ...
        if not lr_read_tables(tabmodule):
            ...
            if start and start not in Prodnames:
                raise_(YaccError,"Bad starting symbol '%s'" % start)
            ...
            if method == 'SLR':
                ...
                raise_(YaccError, "Unknown parsing method '%s'" % method)
