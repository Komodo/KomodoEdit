...
A Komodo extension source dir looks like this:
    ...
    catalog.xml         # XML catalog defining namespace to schema mapping
                        #   for XML autocomplete
                        #   (TODO: would like to move to xmlcatalogs/...)
...
def build_ext(base_dir, log=None):
    ...
    if chrome_dirs:
        ...
        mn_path = tempfile.mktemp()
        fout = open(mn_path, 'w')
        for path in _paths_from_path_patterns([join(jar_build_dir, "*")],
                        excludes=[".svn", ".hg"]):
            relpath = path[len(jar_build_dir)+len(os.sep):]
            fout.write(relpath+'\n')
        fout.close()

        _run_in_dir("zip -r %s.jar -@ < %s" % (ext_info.codename, mn_path), 
