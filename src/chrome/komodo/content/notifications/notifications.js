...
/**
 * The Notification Service API. Exported as `ko.notifications`.
 */
let NotificationSvc = {
  ...
  notify: function NotificationSvc_notify(title, description, iconURL, priority) {
    var notification = new Notification(title, description, iconURL, priority);
  ...
  add: function Notifications_add(notification) {
    ...
    this._observerSvc.notifyObservers(notification,
 ...
 */
let Notification = function(title, description, iconURL, priority, buttons) {
  // Wrap the Notification so that the chrome js code can access it when it is
  // passed as a the subject of a nsIObserverService notification.
  // Cf. https://developer.mozilla.org/En/WrappedJSObject
 ...
 */
let NotificationButton = function(label, accessKey, callback) {
