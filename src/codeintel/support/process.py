...
if sys.platform.startswith("win"):
    import msvcrt
    # WARNING:
    # We are importing 'pywintypes' *before* any of the other win32
    # binary modules to ensure the correct pywintypesXY.dll gets picked
    # up for apps that embed Python and siloed their own Python (e.g.
    # Komodo). If this is not done then the following can happen:
    # 'import win32api' (which itself depends on pywintypesXY.dll) can
    # find the one in C:\WINDOWS\system32 (if that Python version is
    # separately installed) instead of the intended siloed one -- which
    # will likely be in the same directory as the embedding executable.
    # Doing 'import pywintypes' first will allow PyWin32's
    # "pywintypes.py" stub to get used to do the magic of loading the
    # proper pywintypesXY.dll.
    import pywintypes
    import win32api
    import win32file
    import win32pipe
    ...
    def wait(self, timeout=None): 
        ...
        if sys.platform.startswith("win"):
            if timeout is None:
                ...
                # Win32 API's timeout is in millisecs (and an integer)
