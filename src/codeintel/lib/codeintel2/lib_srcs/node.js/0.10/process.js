...
/**
 * Callbacks passed to process.nextTick will usually be called at the end
 * of the current flow of execution, and are thus approximately as fast as
 * calling a function synchronously. Left unchecked, this would starve the
 * event loop, preventing any I/O from occurring.
 */
 ...
 * for exception handling and may be removed in the future. Don&#39;t use
 * it, use domains instead. If you do use it, restart your application
 * after every unhandled exception! Do not use it as the node.js equivalent
 * of On Error Resume Next. An unhandled exception means your application -
 * and by extension node.js itself - is in an undefined state. Blindly
 * resuming means anything could happen. Think of resuming as pulling the
 * power cord when you are upgrading your system. Nine out of ten times
 * nothing happens - but the 10th time, your system is bust. You have been
 * warned.
 * @param err {Error}
 */
