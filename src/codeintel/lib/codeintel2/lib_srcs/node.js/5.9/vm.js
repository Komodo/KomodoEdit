 ...
 * object. script.runInThisContext() runs script&#39;s compiled code and
 ...
 * object. script.runInNewContext() contextifies sandbox if passed or
 * creates a new contextified sandbox if it&#39;s omitted, and then runs
 * script&#39;s compiled code with the sandbox as the global object and
 * returns the result. Running code does not have access to local scope.
 * @param sandbox
 * @param options
 */
 ...
 * contextifiedSandbox and returns the result. Running code does not have
 ...
 * be used in calls to [vm.runInContext()][] or [script.runInContext()][].
 * Inside scripts run as such, sandbox will be the global object, retaining
 * all its existing properties but also having the built-in objects and
 * functions any standard [global object][] has. Outside of scripts run by
 ...
 * via [vm.createContext()][]; it will be used as the global object for
 * code.
 * @param code
 * @param contextifiedSandbox
 * @param options
