...
CSS_MOZ_DATA = {
    ...
    '-moz-appearance':
{       'description': u"The -moz-appearance CSS property is used in Gecko (Firefox) to display an element using a platform-native styling based on the operating system's theme.",
...
{       'description': 'The background-size CSS property specifies the size of the background images. -moz-background-size is supported by Gecko version 1.9.2 (Firefox 3.6). Warning: To support both, Firefox 3.6 and newer versions, you have to include both properties in the stylesheet. See examples.',
        'values': {       'auto': 'Scales the background image in the relevant direction such that its intrinsic proportions are maintained.',
...
{       'description': 'The -moz-binding CSS property is used by Mozilla-based applications to attach an XBL binding to a DOM element.',
        'values': {       'none': 'no XBL binding is applied to the element.'}},
...
{       'description': 'The border-image CSS property allows drawing an image on the borders of elements. This makes drawing complex looking widgets much simpler than it has been and removes the need for nine boxes in some cases.',
        'values': {       'none': 'No image displayed, other border styles are used.',
...
{       'description': 'In Mozilla applications like Firefox, the -moz-border-top-colors CSS property sets a list of colors for the top border.',
        'values': {       'none': 'Default, no colors are drawn or border-color is used, if specified.'}},

    '-moz-box-align':
{       'description': 'In Mozilla applications, -moz-box-align specifies how a XUL box aligns its contents across (perpendicular to) the direction of its layout. The effect of this is only visible if there is extra space in the box.',
        'values': {       'baseline': u"The box aligns the baselines of the contents (lining up the text). This only applies if the box'sorientation is horizontal.",
...
{       'description': 'In Mozilla applications, -moz-box-direction specifies whether a box lays out its contents normally (from the top or left edge), or in reverse (from the bottom or right edge).',
    ...
    '-moz-box-flex':
{       'description': u"In Mozilla applications, -moz-box-flex specifies how a box grows to fill the box that contains it, in the direction of the containing box's layout.",
...
{       'description': 'In Mozilla applications, -moz-box-orient specifies whether a box lays out its contents horizontally or vertically.',
    ...
    '-moz-box-pack':
{       'description': 'In Mozilla applications, -moz-box-pack specifies how a box packs its contents in the direction of its layout. The effect of this is only visible if there is extra space in the box.',
...
{       'description': 'The box-shadow CSS property accepts one or more shadow effects as a comma-separated list. It allows to cast a drop shadow from the frame of almost any arbitrary element. If a border-radius is specified on the element with a box shadow, the box shadow will take on the same rounded corners. The z-ordering of multiple box shadows is the same as multiple text-shadows (the first specified shadow is on top).',
        'values': {       'inset': '(optional). If not specified (default), the shadow is assumed to be a drop shadow (as if the box were raised above the content). The presence of the inset keyword changes the shadow to one inside the frame (as if the content was depressed inside the box). Inset shadows are drawn above background, but below border and content.'},
        'version': 'Firefox (Gecko) 3.5 (1.9.1)'},

    '-moz-box-sizing':
{       'description': '&laquo;  CSS  &laquo;  CSS Reference     &laquo;  CSS Reference:Mozilla Extensions',
        'values': {       }},
...
{       'description': 'In Mozilla applications like Firefox, the -moz-column-gap CSS property sets the gap between columns for block elements which are specified to display as a multi-column element.',
        'values': {       'normal': 'Default value, depends on the user agent. In desktop browsers like Firefox this is 1em. In Gecko 1.8.1 (Firefox 2.0) and before the default value was 0 .'},
        'version': 'Firefox (Gecko) 1.5 (1.8)'},
...
{       'description': 'The opacity CSS property specifies the transparency of an element, i.e. the degree to which the background behind the element is overlaid.',
    ...
    '-moz-outline':
{       'description': '(OBSOLETE) Starting with Gecko 1.8 (Firefox 1.5), the standard CSS 2.1 \n\n outline \n property is supported as well. Use of \n\n outline \n is preferred to  -moz-outline .&nbsp;',
        'values': {       }},
...
{       'description': '(OBSOLETE) Starting with Gecko 1.8 / Firefox 1.5, the standard CSS 2.1 outline-width property is supported as well. Use of outline-width is preferred to -moz-outline-width .',
    ...
    '-moz-padding-end':
{       'description': u"When rendering right-to-left text, -moz-padding-end flips the element's padding without having to specify absolute left or right. In a left-to-right text display -moz-padding-end is treated as a right sided padding, while in a right-to-left display it is padded on the left.",
        'values': {       }},
...
{       'description': '-moz-stack-sizing is an extended CSS property. Normally, a stack will change its size so that all of its child elements are completely visible. For example, moving a child of the stack far to the right will widen the stack so the child remains visible.',
        'values': {       'ignore': u"The stack won't consider this child when calculating the its size.",
                          'stretch-to-fit': u"The child will influence the stack's size."}},

    '-moz-transform':
{       'description': '&laquo;  CSS  &laquo;  CSS Reference     &laquo;  CSS Reference:Mozilla Extensions \n Introduced in Gecko 1.9.1 (Firefox 3.5 / Thunderbird 3 / SeaMonkey 2)',
...
{       'description': u"The  -moz-transform-origin  CSS property lets you modify the origin for transformations of an element.&nbsp; For example, the transform-origin of the  rotate()  function is the centre of rotation.&nbsp; (This property is applied by first translating the element by the negated value of the property, then applying the element's transform, then translating by the property value.)",
        'values': {       'bottom': 'right | right bottom. Same as 100% 100%',
...
{       'description': 'The -moz-transition-delay CSS property specifies the number of seconds to wait between a change being requested to a property that is to be transitioned and the start of the transition effect .',
        'values': {       'time': u"The number of seconds to wait between a property's value changing and the start of the animation effect."},
        'version': 'Firefox (Gecko) 3.7? (Gecko 1.9.3)'},
...
{       },
    ...
    '-moz-user-select':
{       'description': u"Controls the appearance (only)&nbsp;of selection.&nbsp; This does not have any affect on actual selection operation. This doesn't have any effect on content loaded as chrome, except in textboxes.",
...
{       'description': '-moz-window-shadow specifies whether a window will have a shadow. Currently it only works on Mac OS X.',
        'values': {       'default': 'The window will have a shadow with the default window shadow style.',
                          'none': u"The window won't have a shadow."}},
}
