...
static char* _GetOSName(void)
{
    ...
    switch (version) {
        ...
        case 50:
            ...
            break;
        case 49:
            strncpy (osName, "Windows Millennium Edition ", MAX_OS_NAME);
            break;
        case 41:
            strncpy (osName, "Windows 98 ", MAX_OS_NAME);
            if ( osvi.szCSDVersion[1] == 'A' || osvi.szCSDVersion[1] == 'B' ) {
               osName[MAX_OS_NAME] = '\0'; // Ensure NULL termination
               len_left = MAX_OS_NAME - strlen(osName);
               strncat(osName, "SE ", len_left );
            }
            break;
        case 40:
        // Either Win95 or Windows NT
            // XXX - VER_PLATFORM_WIN32_NT and VER_PLATFORM_WIN32_WINDOWS get
            //       mangled by preprocess

            if ( osvi.dwPlatformId == 2) {
            //if ( osvi.dwPlatformId == VER_PLATFORM_WIN32_NT) {
                HKEY hKey;
                char szProductType[WIN_BUFSIZE];
                DWORD dwBufLen = WIN_BUFSIZE;
                LONG lRet;

                strncpy (osName, "Microsoft Windows NT ", MAX_OS_NAME);
                ...
                RegCloseKey( hKey );
                if( (lRet != ERROR_SUCCESS) || (dwBufLen > WIN_BUFSIZE) )
                    break;

                osName[MAX_OS_NAME] = '\0'; // Ensure NULL termination
                len_left = MAX_OS_NAME - strlen(osName);
                if ( lstrcmpi( "WINNT", szProductType) == 0 ) {
                    strncat (osName, "Workstation ", len_left );
                } else if ( lstrcmpi( "LANMANNT", szProductType) == 0 ) {
                    strncat (osName, "Server ", len_left );
                } else if ( lstrcmpi( "SERVERNT", szProductType) == 0 ) {
                    strncat (osName, "Advanced Server ", len_left );
                }

                osName[MAX_OS_NAME] = '\0'; // Ensure NULL termination
                len = strlen(osName);
                len_left = MAX_OS_NAME - len;
            ...
            "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Hotfix\\Q246009",
                                        0, KEY_QUERY_VALUE, &hKey );
                    if ( lRet == ERROR_SUCCESS ) {
                        snprintf (osName + len, len_left, "Service Pack 6a (Build %d)", 
                                  osvi.dwBuildNumber & 0xFFFF );            
                    } else {
                        // Windows NT 4.0 prior to SP6a
                        snprintf (osName + len, len_left, "%s (Build %d)",
            ...
            //} else if ( osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
                strncpy (osName, "Microsoft Windows 95 ", MAX_OS_NAME);
                if (osvi.szCSDVersion[1] == 'C' || osvi.szCSDVersion[1] == 'B') {
                    osName[MAX_OS_NAME] = '\0'; // Ensure NULL termination
                    len_left = MAX_OS_NAME - strlen(osName);
                    strncat(osName, "OSR2 ", len_left );
                }
            }
            break;
    }
